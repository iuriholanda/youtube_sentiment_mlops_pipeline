...
async function fetchComments(videoId) {
    try {
        // We pass the videoId as a query parameter
        const response = await fetch(`${API_URL}/api/get_comments?videoId=${videoId}`);        
        const comments = await response.json();

        if (!response.ok) {
            // Handle errors from your own backend
            console.error("Error from backend:", comments.error);
            outputDiv.innerHTML += `<p>Error fetching comments: ${comments.error}</p>`;
            return [];
        }
